    ...
    return (
        <div className='w-full flex items-center justify-between'>
            <div className='w-full flex items-center'>
                <div
                    className='w-full md:w-auto bg-primary p-2 rounded-lg flex flex-row justify-between md:justify-start items-center pr-0 overflow-hidden'
                >
                    {carImageUrl ? (
                        <img src={carImageUrl} alt='Vehicle'
                             className='h-auto w-40 md:w-80 object-cover rounded-lg mr-0'
                        />
                    ) : null}
                    <div className='flex md:hidden'><CurrentVehicleInfo car={car} /></div>
                    <Link href='/cars'
                          className={`fill-black rounded-full bg-white transform translate-x-1/2 p-1 pr-4 ${url === '/cars' ? 'invisible' : ''}`}
                    >
